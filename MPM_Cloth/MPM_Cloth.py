...
# 先在上面按照自己想法修改简化
# 先修改成3D版本 
# 为什么对于particle1而言也需要更新grid_force?
# 可以不更新grid_force嘛？需要grid force的原因是什么？
...
# 更新grid_force的时候有用到 然后就是求解grid force的方法
@ti.kernel
def Grid_Force():
    ...
    for p in x3:
        ...
        #print("this is dwdx", dw_dx_d)
        #print("this is weight", w)
        ...
        for i, j in ti.static(ti.ndrange(3, 3)):
            ...
            # 这个好像不太对 加减的含义是不重复加减，但是这样不太好 首先这一块至少应该改成有加无减
    ...
    # 这个是force2的值嘛？这个又是在求什么？单独求解二吗 这个地方还是不太明白
    # 这个force2是根据cloth的force进行计算的
    for p in range((ln_type2-2) * N_Line):
        nl = p // (ln_type2-2) # 第几个line
